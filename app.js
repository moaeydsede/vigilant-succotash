...
// Uses window.CRM_CLOUD_DB (initialized in index.html). If unavailable, app falls back to LocalStorage only.
const CLOUD = {
  enabled: false,
  db: null,
...
function cloudInit(){
  const db = window.CRM_CLOUD_DB || null;
...
function cloudDoc(id){
  return CLOUD.db.collection(CLOUD.col).doc(id);
}

async function cloudGet(id, fallback){
  if(!CLOUD.enabled) return fallback;
  try{
    const snap = await cloudDoc(id).get();
    if(!snap.exists) return fallback;
    const data = snap.data() || {};
    return (data.value !== undefined) ? data.value : fallback;
  }catch(e){
    console.warn("cloudGet failed:", id, e);
    return fallback;
  }
}

// Debounced write queue to avoid excessive writes
const __cloudPending = {};
let __cloudTimer = null;

function cloudScheduleSet(id, value){
  if(!CLOUD.enabled) return;
  __cloudPending[id] = value;
  if(__cloudTimer) clearTimeout(__cloudTimer);
  __cloudTimer = setTimeout(async () => {
    const pending = { ...__cloudPending };
    Object.keys(__cloudPending).forEach(k => delete __cloudPending[k]);
    __cloudTimer = null;
    for(const docId of Object.keys(pending)){
      try{
        await cloudDoc(docId).set({ value: pending[docId], updatedAt: Date.now() }, { merge: true });
      }catch(e){
        console.warn("cloudSet failed:", docId, e);
      }
    }
  }, 600);
}
  ...
  function writeLS(key, value){
    ...
    // Cloud mirror for cross-device sync
    if(key === K_USERS) cloudScheduleSet(CLOUD.docs.users, value);
    else if(key === K_COMPANY) cloudScheduleSet(CLOUD.docs.company, value);
    else if(key === K_DB) cloudScheduleSet(CLOUD.docs.data, value);
  }
...
function computeStars(scorePct){
  ...
  function showView(view){
    ...
    $("#btnMenu") && ($("#btnMenu").disabled = (view === "login"));
    $("#btnRecalc") && ($("#btnRecalc").hidden = (view === "login"));
    $("#btnLogout") && ($("#btnLogout").hidden = (view === "login"));
    try{ document.querySelector(".topbar-actions").style.display = (view === "login") ? "none" : ""; }catch(e){}
  ...
  function renderCustomers(){
    ...
    $("#customersList").innerHTML = list.map(c => customerCard(c, "data")).join("") || emptyState("لا توجد نتائج.");
    hookCustomerCardActions($("#customersList"));

    // منع حذف العميل إذا كان لديه أي حركة
    $$("#customersList [data-action='delete']").forEach(btn => {
      const id = btn.dataset.id;
      if(customerHasAnyTx(id)){
        btn.disabled = true;
        btn.title = "لا يمكن حذف العميل لأنه لديه حركات.";
      }
    });
  ...
  function renderRatios(){
    ...
    const q = ($("#ratSearch")?.value || "").trim();
    const sort = ($("#ratSort")?.value || "score_desc");

    // Mode:
    // returns: نسبة (مرتجعات / مبيعات)
    // netpay: نسبة (سداد / صافي المبيعات) حيث صافي المبيعات = مبيعات - مرتجعات - خصومات
    state.ui = state.ui || {};
    const mode = state.ui.ratMode || "returns";

    let list = state.db.customers.slice();

    list = list.map(c => {
      const sales = sumByCustomer(state.db.sales, c.id, from, to);
      const payments = sumByCustomer(state.db.payments, c.id, from, to);
      const returns = sumByCustomer(state.db.returns, c.id, from, to);
      const discounts = sumByCustomer(state.db.discounts, c.id, from, to);

      const retRatio = sales>0 ? (returns/sales*100) : 0;
      const netSales = Math.max(0, sales - returns - discounts);
      const netPayRatio = netSales>0 ? (payments/netSales*100) : 0;

      return {
        ...c,
        _rangeSales: sales,
        _rangePayments: payments,
        _rangeReturns: returns,
        _rangeDiscounts: discounts,
        _rangeRetRatio: retRatio,
        _rangeNetSales: netSales,
        _rangeNetPayRatio: netPayRatio
      };
    });
    ...
    // Sort
    if(sort === "sales_desc"){
      list.sort((a,b)=> (b._rangeSales - a._rangeSales) || ((b.scorePct||0)-(a.scorePct||0)));
    }else if(sort === "returns_asc"){
      list.sort((a,b)=> (a._rangeRetRatio - b._rangeRetRatio) || ((b.scorePct||0)-(a.scorePct||0)));
    }else if(sort === "discounts_asc"){
      list.sort((a,b)=> (a._rangeDiscounts - b._rangeDiscounts) || ((b.scorePct||0)-(a.scorePct||0)));
    }else if(sort === "netpay_desc"){
      list.sort((a,b)=> (b._rangeNetPayRatio - a._rangeNetPayRatio) || ((b.scorePct||0)-(a.scorePct||0)));
    }else{
      list.sort((a,b)=> ((b.scorePct||0)-(a.scorePct||0)) || (b._rangeSales - a._rangeSales));
    }

    // Summary
    const sum = (arr, key) => arr.reduce((a,b)=>a+Number(b[key]||0),0);
    const totalSales = sum(list, "_rangeSales");
    const totalReturns = sum(list, "_rangeReturns");
    const totalDiscounts = sum(list, "_rangeDiscounts");
    const totalPayments = sum(list, "_rangePayments");
    const totalNetSales = Math.max(0, totalSales - totalReturns - totalDiscounts);

    const overallRetRatio = totalSales>0 ? (totalReturns/totalSales*100) : 0;
    const overallNetPayRatio = totalNetSales>0 ? (totalPayments/totalNetSales*100) : 0;

    const summaryEl = $("#ratSummary");
    if(summaryEl){
      const title = (mode==="netpay") ? "صافي المبيعات ↔ السداد" : "المبيعات ↔ المرتجعات";
      const mainVal = (mode==="netpay") ? overallNetPayRatio : overallRetRatio;
      summaryEl.innerHTML = `
        <div class="rat-box">
          <div class="lbl">وضع العرض</div>
          <div class="val">${escapeHtml(title)}</div>
          <div class="hint">الفترة: ${from||"—"} → ${to||"—"}</div>
        </div>
        <div class="rat-box">
          <div class="lbl">إجمالي المبيعات</div>
          <div class="val">${fmtMoney(totalSales)}</div>
          <div class="hint">كل العملاء بعد الفلترة</div>
        </div>
        <div class="rat-box">
          <div class="lbl">إجمالي السداد</div>
          <div class="val">${fmtMoney(totalPayments)}</div>
          <div class="hint">النسبة العامة</div>
        </div>
        <div class="rat-box">
          <div class="lbl">النسبة العامة</div>
          <div class="val">${Number(mainVal||0).toFixed(1)}%</div>
          <div class="hint">${mode==="netpay" ? "سداد ÷ صافي المبيعات" : "مرتجعات ÷ المبيعات"}</div>
        </div>
      `;
    }
    ...
    box.innerHTML = list.map(c => {
      const showRatio = (mode==="netpay") ? c._rangeNetPayRatio : c._rangeRetRatio;
      const ratioLabel = (mode==="netpay") ? "صافي/سداد%" : "مبيعات/مرتجعات%";
      const tmp = {
        ...c,
        // show these ratios on card pills
        payRatio: (mode==="netpay") ? c._rangeNetPayRatio : (c._rangePayments>0 && c._rangeSales>0 ? (c._rangePayments/c._rangeSales*100) : c.payRatio||0),
        retRatio: c._rangeRetRatio
      };

      const extra = `
        <div class="kpi-row" style="margin-top:-2px">
          <span class="kpi-pill"><b>${escapeHtml(ratioLabel)}</b> ${Number(showRatio||0).toFixed(1)}%</span>
          <span class="kpi-pill"><b>مبيعات</b> ${fmtMoney(c._rangeSales||0)}</span>
          <span class="kpi-pill"><b>صافي</b> ${fmtMoney(c._rangeNetSales||0)}</span>
        </div>
        <div class="row gap" style="margin-top:8px;justify-content:flex-end">
          <button class="btn primary" data-action="ratio" data-id="${c.id}">تقارير النِسب للعميل</button>
        </div>
      `;

      return customerCard(tmp, "dash") + extra;
    }).join("") || emptyState("لا توجد نتائج.");
  ...
  function customerCard(c, mode='full'){
    ...
    return `
      <div class="card cust-card ${badge.cls}">
  ...
  function customerHasAnyTx(customerId){
    const db = state.db;
    const lists = [db.sales, db.returns, db.payments, db.discounts];
    return lists.some(arr => (arr||[]).some(x => x.customerId === customerId));
  }
...
function deleteCustomer(customerId){
    ...
    // منع حذف العميل إذا كان عليه أي حركة / إضافة
    if(customerHasAnyTx(customerId)){
      toast("لا يمكن حذف العميل لأنه لديه حركات (مبيعات/مرتجعات/سداد/خصومات). احذف الحركات أولاً.", "err");
      return;
    }

    openModal({
      title: "تأكيد الحذف",
      body: `<div>هل تريد حذف العميل <b>${escapeHtml(c.name)}</b>؟<br><span class="muted small">* لا يمكن الحذف إذا كان لديه أي حركة.</span></div>`,
      foot: [
        mkBtn("حذف", "btn warn", () => {
...
// ====== Reports PDF (period) ======
  ...
  const EXCEL = {
    sheets: {
      Customers: ["المعرف","اسم العميل","الهاتف","المدينة","الحالة","ملاحظات","تاريخ الإضافة","تاريخ التحديث","Score%","النجوم","سداد%","مرتجعات%","صافي/سداد%"],
  ...
  function makeSheetAOA(headers, rows){
    ...
    // AutoFilter on header row (professional Excel feel)
    try{
      const last = headers.length ? (headers.length - 1) : 0;
      const colLetter = (n) => {
        let s = "";
        n = n + 1;
        while(n > 0){
  ...
  function exportCustomersOnly(){
    ...
    const wb = XLSX.utils.book_new();
    const rows = state.db.customers.map(c => {
      const s = sumByCustomer(state.db.sales, c.id);
      const r = sumByCustomer(state.db.returns, c.id);
      const d = sumByCustomer(state.db.discounts, c.id);
      const p = sumByCustomer(state.db.payments, c.id);
      const net = Math.max(0, s - r - d);
      const netPay = net>0 ? (p/net*100) : 0;
  ...
  // ====== Backup / Restore (JSON) for cross-device sync ======
  ...
  function exportBackupJSON(){
    try{
      const payload = {
        app: "Customer Evaluation – CRM Score",
        exportedAt: new Date().toISOString(),
        note: "Backup for moving data between devices (LocalStorage).",
        data: {
          users: readLS(K_USERS, []),
          company: readLS(K_COMPANY, {}),
          db: readLS(K_DB, { customers:[], sales:[], returns:[], payments:[], discounts:[] }),
          log: readLS(K_LOG, []),
          firebase: readLS(K_FIREBASE, {})
        }
      };
      const fname = `CRM_Backup_${todayISO()}_${String(new Date().getHours()).padStart(2,"0")}${String(new Date().getMinutes()).padStart(2,"0")}.json`;
      downloadTextFile(JSON.stringify(payload, null, 2), fname);
      logOp("نسخ احتياطي", "JSON");
      toast("تم إنشاء نسخة احتياطية ✅", "ok");
    }catch(err){
      console.error(err);
      toast("تعذر إنشاء النسخة الاحتياطية.", "bad");
    }
  }
  ...
  async function importBackupJSON(file){
    try{
      ...
      // Restore (ignore session)
      if(obj.data.users) writeLS(K_USERS, obj.data.users);
      if(obj.data.company) writeLS(K_COMPANY, obj.data.company);
      if(obj.data.db) writeLS(K_DB, obj.data.db);
      if(obj.data.log) writeLS(K_LOG, obj.data.log);
      if(obj.data.firebase) writeLS(K_FIREBASE, obj.data.firebase);

      logOp("استعادة", "JSON");
      toast("تمت الاستعادة ✅ سيتم تحديث الصفحة...", "ok");
      setTimeout(() => location.reload(), 700);
    }catch(err){
      console.error(err);
      toast("ملف النسخة الاحتياطية غير صالح.", "bad");
    }
  }
  ...
  function exportAll(){
    ...
    const cRows = state.db.customers.map(c => ([
      ...
      Number(c.payRatio||0),
      Number(c.retRatio||0),
      Number((()=>{ const s=Number(c.totalSales||0); const r=Number(c.totalReturns||0); const d=Number(c.totalDiscounts||0); const p=Number(c.totalPayments||0); const net=Math.max(0,s-r-d); return net>0 ? (p/net*100) : 0; })())
    ]));
  ...
  async function importAllFromFile(file){
    ...
    const counts = {
      ...
      sales: state.db.sales.length,
      returns: state.db.returns.length,
      payments: state.db.payments.length,
      discounts: state.db.discounts.length
    };
    toast(`تم الاستيراد ✅ (عملاء:${counts.customers} • مبيعات:${counts.sales} • مرتجعات:${counts.returns} • سداد:${counts.payments} • خصومات:${counts.discounts})`, "ok");
  }
  ...
  async function importCustomersFromFile(file){
    ...
    recalcAll();
    toast(`تم استيراد العملاء ✅ (عدد: ${state.db.customers.length})`, "ok");
  }
...
// Excel
    ...
    // Backup / Restore JSON (sync between devices)
    $("#btnBackupJSON")?.addEventListener("click", exportBackupJSON);
    $("#btnRestoreJSON")?.addEventListener("click", () => $("#fileRestoreJSON").click());
    ...
    $("#btnRatShowReturns")?.addEventListener("click", () => {
      state.ui = state.ui || {};
      state.ui.ratMode = "returns";
      renderRatios();
      toast("تم عرض: مبيعات ↔ مرتجعات ✅","ok");
    });
    $("#btnRatShowNetPay")?.addEventListener("click", () => {
      state.ui = state.ui || {};
      state.ui.ratMode = "netpay";
      renderRatios();
      toast("تم عرض: صافي المبيعات ↔ السداد ✅","ok");
    });
  ...
  async function start(){
    ...
    // Pull latest cloud data (users/company/db) once at startup, then cache locally
    if(CLOUD.enabled){
      const [cloudUsers, cloudCompany, cloudData] = await Promise.all([
        cloudGet(CLOUD.docs.users, null),
        cloudGet(CLOUD.docs.company, null),
        cloudGet(CLOUD.docs.data, null)
      ]);
      if(cloudUsers && Array.isArray(cloudUsers)) writeLS(K_USERS, cloudUsers);
      if(cloudCompany && typeof cloudCompany === "object") writeLS(K_COMPANY, cloudCompany);
      if(cloudData && typeof cloudData === "object") writeLS(K_DB, cloudData);
    }

    loadAll();

    // If cloud is empty, seed it with local defaults once
    if(CLOUD.enabled){
      const localUsers = readLS(K_USERS, [DEFAULT_ADMIN]);
      const localCompany = readLS(K_COMPANY, { ...DEFAULT_COMPANY });
      const localDb = readLS(K_DB, { ...DEFAULT_DB });
      cloudScheduleSet(CLOUD.docs.users, localUsers);
      cloudScheduleSet(CLOUD.docs.company, localCompany);
      cloudScheduleSet(CLOUD.docs.data, localDb);
    }
