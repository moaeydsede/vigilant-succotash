  ...
  function customerCard(c, mode='full'){
    ...
    const kpis = (showTotals || showDash) ? `
      <div class="kpi-row">
        ...
        <span class="kpi-pill"><b>النجوم</b> <span class="stars">${stars}</span></span><button class="pill-btn" data-action="ratio" data-id="${c.id}">نِسب</button>
      </div>
    ` : "";
...
// ====== Reports PDF (period) ======
  ...
  function wire(){
    ...
    // === Ratio buttons (explicit) ===
    $("#btnRatioReturns")?.addEventListener("click", ()=> pdfRatioReport("returns"));
    $("#btnRatioPayments")?.addEventListener("click", ()=> pdfRatioReport("payments"));
    $("#btnRatioDiscounts")?.addEventListener("click", ()=> pdfRatioReport("discounts"));
