...
(() => {
  ...
  function customerCard(c, compact=false){
    ...
    const salesT = sumByCustomer(state.db.sales, c.id);
    const returnsT = sumByCustomer(state.db.returns, c.id);
    const paymentsT = sumByCustomer(state.db.payments, c.id);
    const discountsT = sumByCustomer(state.db.discounts, c.id);
    const balanceT = salesT - (returnsT + paymentsT + discountsT);
    ...
    const extra = compact ? "" : `
      ...
      <div class="meta">
        <span class="chip">مبيعات: ${fmtMoney(salesT)}</span>
        <span class="chip">سداد: ${fmtMoney(paymentsT)}</span>
        <span class="chip">مرتجعات: ${fmtMoney(returnsT)}</span>
        <span class="chip">خصم: ${fmtMoney(discountsT)}</span>
        <span class="chip">الرصيد: ${fmtMoney(balanceT)}</span>
      </div>
      ...
      <div class="card-actions">
        ...
        <button class="btn primary" data-action="ratio" data-id="${c.id}">تقرير النِسب</button>
  ...
  async function pdfCustomerStatement(){
    ...
    // Merge transactions for this customer
    const merged = []
      .concat(state.db.sales.map(x => ({...x, _type:"sale", _label:"مبيعات", _dc:"debit"})))
      .concat(state.db.returns.map(x => ({...x, _type:"return", _label:"مرتجعات", _dc:"credit"})))
      .concat(state.db.payments.map(x => ({...x, _type:"payment", _label:"سداد", _dc:"credit"})))
      .concat(state.db.discounts.map(x => ({...x, _type:"discount", _label:"خصم", _dc:"credit"})))
      .filter(x => x.customerId === customerId)
      .filter(x => inRange(x.date, from, to))
      .sort((a,b) => (a.date||"").localeCompare(b.date||"") || (a.createdAt||0)-(b.createdAt||0));

    let balance = 0; // مبيعات - (سداد + مرتجعات + خصم)
    ...
    doc.autoTable({
      ...
      body: rows.length ? rows : [["", "", "لا توجد حركات ضمن الفترة", "", "", "", "", ""]],
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text(`إجمالي مبيعات: ${s.toFixed(2)}   |   إجمالي سداد: ${p.toFixed(2)}   |   إجمالي مرتجعات: ${r.toFixed(2)}   |   إجمالي خصم: ${d.toFixed(2)}`, 40, y);
    doc.text(`الرصيد النهائي (مبيعات - سداد - مرتجعات - خصم): ${net.toFixed(2)}`, 40, y + 16);

    // Stars & score
    doc.setFont("helvetica", "normal");
    doc.text(`التقييم: ${"⭐".repeat(clamp(c.stars||1,1,5))}   |   Score: ${Number(c.scorePct||0).toFixed(1)}%`, 40, y + 34);
  ...
  async function pdfRatioReport(kind){
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.text(`الفترة: من ${from||"—"} إلى ${to||"—"}`, 40, 72);

    const head = [["#", "العميل", "المدينة", "المبيعات", info.label, "النسبة%", "النجوم", "Score%"]];
    const body = info.rows.slice(0, 200).map((x,i)=>[
      i+1,
      x.name,
      x.city,
      x.sales.toFixed(2),
    ...
    const fname =
      kind === "returns" ? "Sales_vs_Returns" :
      kind === "payments" ? "Sales_vs_Payments" :
      "Sales_vs_Discounts";

    doc.save(`${fname}_${from||"all"}_${to||"all"}.pdf`);
    logOp("طباعة PDF", titleCore);
  }
  ...
  function wire(){
    ...
    // تقارير النِسب من صفحة التقارير (حسب نفس الفترة)
    $("#btnRepSalesReturnsPdf").addEventListener("click", () => {
      // استخدم نفس نطاق التقارير
      const from = $("#repFrom").value; const to = $("#repTo").value;
      // استخدم نفس وظيفة التقرير ولكن بحقن الفترة عبر حقول صفحة PDF إن لم تكن موجودة
      try{ $("#pdfRepFrom").value = from; $("#pdfRepTo").value = to; }catch(e){}
      pdfRatioReport("returns");
    });
    $("#btnRepSalesPaymentsPdf").addEventListener("click", () => {
      const from = $("#repFrom").value; const to = $("#repTo").value;
      try{ $("#pdfRepFrom").value = from; $("#pdfRepTo").value = to; }catch(e){}
      pdfRatioReport("payments");
    });
    $("#btnRepSalesDiscountsPdf").addEventListener("click", () => {
      const from = $("#repFrom").value; const to = $("#repTo").value;
      try{ $("#pdfRepFrom").value = from; $("#pdfRepTo").value = to; }catch(e){}
      pdfRatioReport("discounts");
    });
