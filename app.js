...
const APP_VERSION = 'v2.3.0 VIP';
  ...
  function showRoute(route){
    ...
    $$('.tab-item[data-route]').forEach(b=>b.classList.toggle('active', b.dataset.route===route));
    closeSidebar();
  ...
  $$('.tab-item[data-route]').forEach(b=>b.addEventListener('click', ()=>showRoute(b.dataset.route)));
  const tabMore = document.getElementById('tabMore');
  if(tabMore){ tabMore.addEventListener('click', openSidebar); }
  ...
  function customerCard(c, compact=false){
    ...
    const el=document.createElement('div');
    el.className='row vip-row';
        const initial = (c.name||'؟').trim().charAt(0).toUpperCase();
    const statusTxt = c.status==='Active'?'نشط':'غير نشط';
    const meta = `${escapeHtml(c.city||'—')} • ${statusTxt}`;
    const pay = clamp(c.payRatio||0,0,999);
    const ret = clamp(c.retRatio||0,0,999);
    el.innerHTML = `
      <div class="row-left">
      ...
      <div class="row-main">
        <div class="row-title">
          ...
          ${ (c.note && String(c.note).trim()) ? `<span class="note-dot" title="يوجد ملاحظة">📝</span>` : `` }
        </div>
        <div class="row-sub">${meta}</div>
        <div class="row-metas">
          <span class="meta-chip">سداد <b>${pay}%</b></span>
          <span class="meta-chip">مرتجعات <b>${ret}%</b></span>
        </div>
      </div>
      <div class="row-right">
        <div class="score-box">
          <div class="stars">${starsText(c.stars||scoreToStars(c.scorePct||0))}</div>
          <div class="pct">${Number(c.scorePct||0)}%</div>
        </div>
        <span class="badge ${badge.cls}">${badge.label}</span>
        ${compact?'':`<button class="btn-mini" data-act="edit" aria-label="تعديل">✎</button>`}
  ...
  function txCard(x){
    ...
    const badgeCls = x.__type==='sales'?'good':x.__type==='payments'?'vip':x.__type==='returns'?'danger':'warn';
        const typeTxt = txLabel(x.__type);
    const name = escapeHtml(c?.name||'—');
    const inv = escapeHtml(x.invoiceNo||'—');
    const dt = escapeHtml(x.date||'—');
    const note = escapeHtml(x.note||'');
    el.innerHTML = `
      <div class="row-left">
        <div class="tx-ic ${x.__type}">${x.__type==='sales'?'🧾':x.__type==='payments'?'💳':x.__type==='returns'?'↩️':'🏷️'}</div>
      </div>
      <div class="row-main">
        <div class="row-title"><span class="nm">${typeTxt}</span> <span class="muted">•</span> ${name}</div>
        <div class="row-sub">${inv} • ${dt}${note?` • ${note}`:''}</div>
      </div>
      <div class="row-right">
        <span class="amount badge ${badgeCls}">${fmt(x.amount||0)}</span>
        <button class="btn-mini danger" data-act="del" aria-label="حذف">🗑️</button>
      </div>`;
